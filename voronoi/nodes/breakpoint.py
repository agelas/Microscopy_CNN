...
class Breakpoint:
    '''
    A breakpoint between two arcs on the beach line.
    '''
    ...
    def __init__(self, breakpoint: tuple, edge=None):
        ...
        breakpoint and p_j defines the parabola to the right. An internal node v in the AVL (BBST) tree representing 
        the beach line has a pointer to the half edge in the doubly connected edge list of the Voronoi diagram. Ie,
        v has a pointer to one of the half-edges of the edge being traced out by the breakpoint represented by v.
    ...
    def get_intersection(self, l, max_y=None):
        ...
        # Handle cases where one point's y-coordinate is the same as the sweep line
            ...
            # We now need to solve for x with ugly math
            x = -(math.sqrt(
